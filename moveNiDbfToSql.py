...
conex = changeDatabase(args.serverTyp, args.hostIp, args.interMedDb, args.adminUser,args.adminPass)
dirpath = os.getcwd()
...
for file in glob.glob("*.dbf"):
    ...
    tableCreate = 'create table ' + args.interMedDb + '.dbo.vw_origin_%s (%s)' % (
    ...
    try:
        ...
        for i,rec in enumerate(table):
            ...
            #listoflists = list()
            if fileName != 'marca' and i % 999 == 0 and i != 0 and listoflists:
                queryInsertBig(conex,sql,listoflists, 'inserting data')
                listoflists=list()
            elif i % 29 == 0 and i != 0 and listoflists:
                queryInsertBig(conex,sql,listoflists, 'inserting data')
                listoflists=list()
        if listoflists:
